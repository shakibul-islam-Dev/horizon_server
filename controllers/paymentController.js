...
exports.createPaymentIntent = async (req, res, params, body, user) => {
...
exports.getMyPayments = async (req, res, params, body, user) => {
  if (!user) return sendError(res, 401, 'Not authenticated');
  const url = new URL(req.url, `http://${req.headers.host}`);
  const { page, limit, skip } = paginate(url.searchParams.get('page'), url.searchParams.get('limit'));
  const [payments, total] = await Promise.all([
    Payment.find({ user: user.userId }).populate('item', 'title price images').sort({ createdAt: -1 }).skip(skip).limit(limit).lean(),
    Payment.countDocuments({ user: user.userId }),
  ]);
  sendSuccess(res, 'Payments fetched', payments, buildMeta(page, limit, total));
};

exports.getPaymentById = async (req, res, params, body, user) => {
  if (!user) return sendError(res, 401, 'Not authenticated');
...
exports.getPaymentsByItem = async (req, res, params, body, user) => {
  if (!user) return sendError(res, 401, 'Not authenticated');
  const url = new URL(req.url, `http://${req.headers.host}`);
  const { page, limit, skip } = paginate(url.searchParams.get('page'), url.searchParams.get('limit'));
  const [payments, total] = await Promise.all([
    Payment.find({ item: params.itemId }).populate('user', 'name image').sort({ createdAt: -1 }).skip(skip).limit(limit).lean(),
    Payment.countDocuments({ item: params.itemId }),
  ]);
  sendSuccess(res, 'Payments fetched', payments, buildMeta(page, limit, total));
};

exports.createPayment = async (req, res, params, body, user) => {
  if (!user) return sendError(res, 401, 'Not authenticated');
  ...
  if (!item) return sendError(res, 404, 'Item not found');
  if (item.status !== 'active') return sendError(res, 400, 'Item not available');
  const payment = await Payment.create({ ...body, user: user.userId, status: 'pending' });
  await payment.populate('item', 'title price images');
  sendCreated(res, 'Payment created', payment);
};

exports.updatePaymentStatus = async (req, res, params, body, user) => {
  if (!user) return sendError(res, 401, 'Not authenticated');
  const payment = await Payment.findById(params.id);
  if (!payment) return sendError(res, 404, 'Payment not found');
  if (payment.user.toString() !== user.userId) return sendError(res, 403, 'Not authorized');
  if (!body || !body.status) return sendError(res, 400, 'Status is required');
  const valid = { pending: ['completed', 'failed'], completed: ['refunded'], failed: ['pending'], refunded: [] };
...
// ===== TRANSACTIONS =====
exports.getBuyerTransactions = async (req, res, params, body, user) => {
  if (!user) return sendError(res, 401, 'Not authenticated');
  const url = new URL(req.url, `http://${req.headers.host}`);
  const { page, limit, skip } = paginate(url.searchParams.get('page'), url.searchParams.get('limit'));
  const [transactions, total] = await Promise.all([
    Transaction.find({ buyer: user.userId }).populate('seller', 'name image').populate('item', 'title price images').populate('payment', 'status paymentMethod').sort({ createdAt: -1 }).skip(skip).limit(limit).lean(),
    Transaction.countDocuments({ buyer: user.userId }),
  ]);
  sendSuccess(res, 'Transactions fetched', transactions, buildMeta(page, limit, total));
};

exports.getSellerTransactions = async (req, res, params, body, user) => {
  if (!user) return sendError(res, 401, 'Not authenticated');
  const url = new URL(req.url, `http://${req.headers.host}`);
  const { page, limit, skip } = paginate(url.searchParams.get('page'), url.searchParams.get('limit'));
  const [transactions, total] = await Promise.all([
    Transaction.find({ seller: user.userId }).populate('buyer', 'name image').populate('item', 'title price images').populate('payment', 'status paymentMethod').sort({ createdAt: -1 }).skip(skip).limit(limit).lean(),
    Transaction.countDocuments({ seller: user.userId }),
  ]);
  sendSuccess(res, 'Transactions fetched', transactions, buildMeta(page, limit, total));
};

exports.getTransactionById = async (req, res, params, body, user) => {
  if (!user) return sendError(res, 401, 'Not authenticated');
...
exports.createTransaction = async (req, res, params, body, user) => {
  if (!user) return sendError(res, 401, 'Not authenticated');
...
exports.updateTransactionStatus = async (req, res, params, body, user) => {
  if (!user) return sendError(res, 401, 'Not authenticated');
  if (!body || !body.status) return sendError(res, 400, 'Status is required');
  const t = await Transaction.findById(params.id);
  if (!t) return sendError(res, 404, 'Transaction not found');
